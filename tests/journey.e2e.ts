...
async function status(page:Page,nextFrame=false){return page.evaluate(async frame=>{if(frame)await new Promise<void>(resolve=>requestAnimationFrame(()=>resolve()));const g=(window as unknown as TestWindow).__asterfall;return {position:g.player.position.toArray() as number[],yaw:g.player.orbitYaw as number,state:g.state,panel:g.panel,cinema:Boolean(g.cinema)};},nextFrame);}
...
async function walk(page:Page,x:number,z:number,climb=false){
  const initial=await status(page);const allowance=Math.max(10,Math.hypot(x-initial.position[0]!,z-initial.position[2]!)/2);
  const held=new Set<string>();if(climb){await page.keyboard.down('Space');held.add('Space');}
  try{for(;;){const s=await status(page,true);if(s.cinema)return;if(s.panel!=='none')throw new Error(`Walking interrupted: ${s.panel} ${s.state.quest}`);const dx=x-s.position[0]!,dz=z-s.position[2]!;if(Math.hypot(dx,dz)<.7)return;
    if(s.state.playSeconds-initial.state.playSeconds>allowance)throw new Error(`Could not walk to ${x},${z}: ${JSON.stringify(s)}`);
    ...
    for(const key of held)if(!next.has(key)){await page.keyboard.up(key);held.delete(key);}for(const key of next)if(!held.has(key)){await page.keyboard.down(key);held.add(key);}
