...
export function advanceClock(state: GameState, dt: number, countPlayTime = true): boolean {
  if (countPlayTime) state.playSeconds += dt;
  state.time += dt * 24 / CONFIG.dayDuration;
  while (state.time >= 24) {
    state.time -= 24;
    state.day++;
    if (state.day % 2 === 0) {
      state.weatherSeed = (Math.imul(state.weatherSeed, 1664525) + 1013904223) >>> 0;
      state.weather = ['clear', 'cloudy', 'rain'][state.weatherSeed % 3] as GameState['weather'];
    }
    if (state.day % CONFIG.bloodMoonDays === 0 && state.lastBloodMoon !== state.day) state.flags.bloodMoonPending = true;
  }
